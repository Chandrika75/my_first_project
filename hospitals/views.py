...
def add_doctor(request):
    ...
    if request.method == 'POST':
        ...
        dt = request.POST['Days']
        try:
            Doctor.objects.create(name=n, mobile=m, special=sp, Days=dt)
            error = "no"
        except:
            error = "yes"
...
def add_patient(request):
    ...
    if request.method == 'POST':
        ...
        try:
            Patient.objects.create(name=n, gender=g, mobile=m, address=a, age=e, disease=dis)
            error = "no"
        except:
            error = "yes"
...
def edit_Appointment(request, pid):
    ...
    if request.method == "POST":
        d2 = request.POST['doctor']
        p2 = request.POST['patient']
        d3 = request.POST['date']
        t3 = request.POST['time']

        Appointment.doctor = d2
        Appointment.patient = p2
        Appointment.date = d3
        Appointment.time = t3
        ...
        try:
            ...
            error = "yes"
    return render(request, 'edit_Appointment.html', locals())
